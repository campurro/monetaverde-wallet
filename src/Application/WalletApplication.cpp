...
const char BYTECOIN_URI_SCHEME_NAME[] = "monetaverde";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("monetaverdewallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("monetaverdewallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Monetaverde wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Monetaverde wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have monetaverded with non-default RPC port, you should terminate it and relaunch Monetaverde Wallet\n"
      "or\n"
      "Set the Local deamon required port in Monetaverde Wallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
