...
const char BLOG_RSS_LINK_TAG_NAME[] = "link";
const char BLOG_RSS_LINK_HREF_ATTRIBUTE_NAME[] = "";
const char BLOG_RSS_TITLE_TAG_NAME[] = "title";
const char BLOG_RSS_CONTENT_TAG_NAME[] = "description";
...
void BlogReader::processBlogReplyData(const QString& _data) {
  ...
  while (!xml.atEnd() && newMessages.size() < MAX_MESSAGE_COUNT) {
    xml.readNext();
    if (xml.isStartElement() && !xml.name().compare(QString("item"))) {
      MessageItem messageItem;
      // wordpress send 3 URL By default so only the first goes on the right place
      bool takeFirstUrl(false);
      while (!xml.atEnd() && !(xml.isEndElement() && !xml.name().compare(QString("item")))) {
        xml.readNext();
        if (xml.isStartElement()) {
          if (!xml.name().compare(QString(BLOG_RSS_ID_TAG_NAME))) {
            messageItem.messageId = xml.readElementText();
              ...
              if (0 < strlen(BLOG_RSS_LINK_HREF_ATTRIBUTE_NAME)) {
      ...
      newMessages.prepend(messageItem);
    }
  }
