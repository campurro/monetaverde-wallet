...
const char DEFAULT_WALLET_FILE_NAME[] = "monetaverdewallet.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("monetaverdewallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("monetaverdewallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("monetaverdewallet.desktop");
  ...
  res = autorunSettings.contains("BytecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("MonetaverdeWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.monetaverde.monetaverdewallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Monetaverde Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("MonetaverdeWallet", appPath);
    } else {
      autorunSettings.remove("MonetaverdeWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\monetaverde", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:monetaverde");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\monetaverde\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\monetaverde\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("monetaverdewallet.cfg"));
